...
export const BASE = { 1:420, 2:8500, 3:74000, 4:280, 5:310, 6:4200, 7:2800, 8:3900, 9:1800 };

// ── Trading state helpers ─────────────────────────────────────────────────────
const STARTING_BALANCE = 15000;
let _orderId = 1000;
let _tradeId = 1000;
export const newOrderId  = () => `ORD-${++_orderId}`;
export const newTradeId  = () => `TRD-${++_tradeId}`;
...
export function matchOrders(orders, marketPrices, holdings, balance){
...
// ── Themes ───────────────────────────────────────────────────────────────────
export const DK={bg:"#070a0e",bg2:"#080c09",bg3:"#0a0f0a",bdr:"#0f1f0f",bdr2:"#1a2e1a",txt:"#a8b8a0",txtD:"#2a5a2a",txtM:"#4a8a4a",acc:"#00ff50",accD:"#00cc40",hdrBg:"#080c08",stBg:"#0a100a",buyT:"#00ff50",sellT:"#ff4040",askT:"#cc3535",bidT:"#00cc40",inBg:"#0a0f0a",inBdr:"#1a2a1a",rowHov:"rgba(0,255,80,0.04)",shad:"0 1px 12px rgba(0,0,0,0.5)"};
export const LT={bg:"#f0f4f0",bg2:"#ffffff",bg3:"#f8faf8",bdr:"#dde8dd",bdr2:"#b8d4b8",txt:"#2a3a2a",txtD:"#7a9a7a",txtM:"#3a7a3a",acc:"#16803a",accD:"#15803d",hdrBg:"#ffffff",stBg:"#f0f6f0",buyT:"#15803d",sellT:"#dc2626",askT:"#dc2626",bidT:"#15803d",inBg:"#f8faf8",inBdr:"#c5d8c5",rowHov:"rgba(22,128,58,0.04)",shad:"0 1px 6px rgba(0,0,0,0.08)"};
export const MONO="'Share Tech Mono','Courier New',monospace";
export const ORB="'Orbitron',sans-serif";

// ── Portfolio ─────────────────────────────────────────────────────────────────
export function Portfolio({D,dark,holdings=[],tradeHistory=[],dbCards=[],isMobile=false,onNavigateToMarket}){
    ...
    <div style={{flex:1,overflowY:"auto",padding:isMobile?"12px":"20px",display:"flex",flexDirection:"column",gap:"16px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:"16px"}}>
        ...
        <div style={{background:D.bg2,border:`1px solid ${D.bdr}`,borderRadius:"6px",overflow:"hidden"}}>
          ...
          {watchlist.length===0&&(
            <div style={{padding:"32px",textAlign:"center",color:D.txtD,fontSize:"14px"}}>No cards on watchlist — browse cards and add them here</div>
          )}
        </div>
      </div>
    </div>
...
// ── Orders ────────────────────────────────────────────────────────────────────
export function Orders({D,dark,orders=[],onCancel,dbCards=[],isMobile=false}){
...
// ── History ───────────────────────────────────────────────────────────────────
export function History({D,dark,tradeHistory=[],ledger=[],dbCards=[],isMobile=false}){
...
// ── Auth Modal ────────────────────────────────────────────────────────────────
export function AuthModal({D,dark,onClose,onAuth}){
...
// ── Waitlist Form ─────────────────────────────────────────────────────────────
export function WaitlistForm({D,dark,isMobile=false,onLoginClick}){
  const [name,setName]=useState("");
  const [email,setEmail]=useState("");
...
// ── Landing Page ──────────────────────────────────────────────────────────────
const GAME_NAV=[
  {name:"Pokémon",      sets:["Base Set","Jungle","Fossil","Neo Genesis","Skyridge","CoroCoro Promos"]},
  {name:"MTG",          sets:["Alpha","Beta","Unlimited","Revised","Arabian Nights","Legends","The Dark"]},
  {name:"Yu-Gi-Oh!",   sets:["Legend of Blue Eyes","Metal Raiders","Spell Ruler","Pharaoh's Servant","LOB"]},
  {name:"One Piece TCG",sets:["Romance Dawn","Paramount War","Pillars of Strength","Kingdom of Intrigue"]},
  {name:"Flesh and Blood",sets:["Welcome to Rathe","Arcane Rising","Crucible of War","Monarch","Everfest"]},
...
export function Landing({D,dark,dbCards,onEnterDemo,onOpenAuth}){
  const isMobile=useIsMobile();
...
// ── Ticker ────────────────────────────────────────────────────────────────────
export function Ticker({D,dark,tradeHistory=[],dbCards=[],marketPrices={}}){
...
// ── Notification system ────────────────────────────────────────────────────────
export function NotificationBell({D,dark,notifications=[],onClear,onClearAll}){
...
// ── CSV Import Modal ───────────────────────────────────────────────────────────
export function CSVImportModal({D,dark,dbCards=[],onImport,onClose,marketPrices={},tradeHistory=[]}){
...
export function getRepTier(tradeCount=0){
  return [...REP_TIERS].reverse().find(t=>tradeCount>=t.min)||REP_TIERS[0];
}
export function RepBadge({tradeCount=0,size="sm"}){
...
export function ProfileSettings({D,dark,user,profile,tradeHistory=[],holdings=[],balance=0,onProfileUpdate,onDarkToggle,isMobile=false,onNotifPrefsChange}){
